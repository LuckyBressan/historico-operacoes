...
type statusType = "pago" | "pendente" | "atrasado" | "concluido" | "ativo";
...
export default function ContratosList({
  ...
  const getVariantBadgeStatus = (status: statusType) => {
    switch (status) {
      case "pago":
      case "concluido":
        return "primary";
      ...
      <div className="space-y-4">
        <Accordion
          ...
          {contratos.map((contrato) => {
            const statusContrato: statusType =
              contrato.status == ContratoStatusEnum.ATIVO ? "ativo" : "concluido";
            return (
              <AccordionItem
                      ...
                      ">
                        ...
                        <div className="col-2 md:col-3 md:justify-self-end flex items-center gap-6">
                          ...
                          <BadgeSituacao
                            ...
                            variant={getVariantBadgeStatus(statusContrato)}>
                            {statusContrato}
                          </BadgeSituacao>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="overflow-hidden">
                          <Table>
                            ...
                            <TableBody>
                              {parcelas[contrato.contrato]?.map(
                                (parcela, id) => {
                                  const parcelaPendente =
                                    parcela.capitalaberto > 0;

                                  const dataVencimento = new Date(
                                    parcela.datavencimento
                                  );

                                  let status: statusType = parcelaPendente
                                    ? "pendente"
                                    : "pago";
